...
export function useTransportStore() {
  ...
  useEffect(() => {
    function handleNuiMessage(event: MessageEvent) {
      ...
      if (message.type === "TS_SEARCH_RESULT") {
        const vehicles =
          (message.payload && Array.isArray(message.payload.vehicles)) ? message.payload.vehicles :
          (typeof (message as any).data !== "undefined" && Array.isArray((message as any).data.vehicles)) ? (message as any).data.vehicles : [];
        setVehicleListState(vehicles);
        if (vehicles.length === 0) setUiState({ status: "empty" });
        else if (vehicles.length === 1) setUiState({ status: "success", vehicle: vehicles[0] });
  ...
  const selectedVehicle = useMemo(() => (hasSelectedVehicle ? uiState.vehicle : null), [hasSelectedVehicle, uiState]);
  const vehicleList = useMemo(() => (uiState.status === "list" ? uiState.vehicles : vehicleListState), [uiState, vehicleListState]);
  ...
  const transferVehicleOwner = useCallback(async (vehicleId: string, newOwnerId: string, firstname?: string, lastname?: string) => {
    const response = await fetchNui("ts_transferOwner", { vehicleId, newOwnerId, firstname, lastname });
    if (response && response.vehicles && response.vehicles[0]) {
      setVehicleListState((prevList) => prevList.map(vehicle => vehicle.id === response.vehicles[0].id ? response.vehicles[0] : vehicle));
      setUiState({ status: "success", vehicle: response.vehicles[0] });
    }
  }, []);

  const reportVehicleMileage = useCallback(async (vehicleId: string, newMileage: number) => {
    const response = await fetchNui("ts_reportMileage", { vehicleId, newMileage });
    if (response && Array.isArray(response.vehicles) && response.vehicles[0]) {
      setVehicleListState((prevList) => prevList.map(vehicle => vehicle.id === response.vehicles[0].id ? response.vehicles[0] : vehicle));
      setUiState({ status: "success", vehicle: response.vehicles[0] });
    } else if (response && response.vehicle) {
      setVehicleListState((prevList) => prevList.map(vehicle => vehicle.id === response.vehicle.id ? response.vehicle : vehicle));
      setUiState({ status: "success", vehicle: response.vehicle });
    } else if (response && Array.isArray(response.vehicles) && response.vehicles.length === 0) {
